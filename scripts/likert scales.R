...
likert_data <- read.csv("data/uncertainty_data_codes.csv")


colnames(likert_data)
unique(likert_data$often_stakeholders)
...
likert_often <- likert_data %>% 
            select(often_environment:often_funding)
         
unique(likert_often$often_evidence)
